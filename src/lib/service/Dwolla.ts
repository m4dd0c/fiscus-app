...
export async function createDwollaCustomer(
  ...
  try {
    ...
    console.log("Dwolla API Error:", error.response?.data || error.message);
    throw new Error(
...
async function findFundingSource(customerUrl: string, accountNumber: string) {
  try {
    ...
    console.log(
      {
        self: fundingSources._link.self,
        customer: fundingSources._link.customer,
      },
      "fs",
    );
    ...
    console.log("Error checking funding sources:", error?.response?.data);
    return null;
  }
}
